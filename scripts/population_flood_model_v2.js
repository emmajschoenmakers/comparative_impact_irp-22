...
//Sum pixel values of exposed population raster in Sofala province
...
print('Population Exposed Sofala : ', ee.Number(sofala_number_pp_exposed));  
print(' Population Exposed Zambezia', ee.Number(zambezia_number_pp_exposed));
...
var pop_exp_sofala = population_exposed.clip(sofala);
var pop_exp_zambezia = population_exposed.clip(zambezia);
...
var text1 = ui.Label('Flood Impact between:',textVis);
...
var text2 = ui.Label('Estimated flood impact in Sofala, Mozambique:',textVis);
var text2_2 = ui.Label('Please wait...',subTextVis);
dates(after_collection).evaluate(function(val){text2_2.setValue('based on World Population Hub data &  Sentinel-1 imagery '+val)});
var number2 = ui.Label('Please wait...',numberVIS); 
ee.Number(sofala_number_pp_exposed).evaluate(function(val){number2.setValue(val+' people')}),numberVIS;
//flood_area_ha.evaluate(function(val){number2.setValue(val+' hectares')}),numberVIS;
var text3 = ui.Label('Scale: yellow - low impact, orange - medium impact, red - high impact',textVis);
...
var colorBar = ui.Thumbnail({
  ...
  params: makeColorBarParams(populationExposedVis.palette),
  style: {position: 'bottom-left', stretch: 'horizontal', margin: '0px 8px', maxHeight: '24px'},
});

// Legend title
var legendTitle = ui.Label({
  value: 'Legend: Impacted areas (people/km²)',
...
var legendLabels = ui.Panel({
  widgets: [
    ui.Label(populationExposedVis.min, {margin: '4px 8px'}),
    ui.Label(
        ((populationExposedVis.max-populationExposedVis.min) / 2+populationExposedVis.min),
        {margin: '4px 8px', textAlign: 'center', stretch: 'horizontal'}),
    ui.Label(populationExposedVis.max, {margin: '4px 8px'})
  ],
...
Export.image.toAsset({
  image: pop_exp_sofala,
  description: 'exporting-population-sofala-map-to-Assest',
  assetId: 'sofala-exppop-map-mozambique',
  scale: 20,
 ...
 //Export the image to an Earth Engine asset.
Export.image.toAsset({
  image: pop_exp_zambezia,
  description: 'exporting-population-zambezia-map-to-Asset',
  assetId: 'zambezia-exppop-map-mozambique',
  scale: 20,
 ...
 //Export the reprojected Flood image at 1KM RES to an Earth Engine asset.
 // to be used later for the synergy of all datasets 
 
 
Export.image.toAsset({
  image: flooded_res1,
  description: 'exporting-reprojected-flood-moz-map-to-Asset',
  assetId: 'reprojected-1km-flood-map-mozambique',
  scale: 1000,
...
Export.image.toDrive({
  image: flooded_res1,
  description: 'moz-res1-flooded-toDrive',
  scale: 1000,
  fileFormat: 'GeoTIFF',
  maxPixels: 1e13
});
